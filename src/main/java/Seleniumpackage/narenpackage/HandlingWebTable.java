...
public class HandlingWebTable {
    ...
    void getBrowser(String browser) throws InterruptedException {
        ...
        for (int i = 1; i <= headerList.size(); i++) {
            ...
            System.out.println("Header column name is :: " + headrName);
        }
        ...
        for (int row = 1; row <= rowsList.size(); row++) {
            ...
            for (int col = 1; col <= colList.size(); col++) {
                String colData = driver.findElement(By.xpath("//table[@class='dataTable']/tbody/tr[" + row + "]/td[" + 1 + "]")).getText();
                if(colData.equals("Tamboli Capital") && colData.equals("X")){
